...
def start_block_server ():
    global server_addr
    server = TCPSocketEngine (iopoll.Poll (), debug=False)
...
def start_unblock_server ():
    ...
    if 'EPoll' in dir(iopoll):
        ...
        poll = iopoll.Poll ()
    server = TCPSocketEngine (poll, debug=False)
    server.set_logger (getLogger ("server"))
    server.get_time = tc.time
    def _accept_client (sock, *args):
        sock.setblocking (0)
        return sock
    ...
    def _on_recv (conn):
        ...
        return
    server.listen_addr (server_addr, server.read_unblock, (len(data), _on_recv, None), new_conn_cb=_accept_client)
